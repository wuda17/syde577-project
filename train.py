...
parser.add_argument("--database_path", type=str, help="training data location")
parser.add_argument("--saved_models_path", type=str, help="path where to save models")
parser.add_argument("--experiment_name", type=str, help="name of the experiment")
parser.add_argument(
    "--num_workers", type=int, default=0, help="number of parallel CPU workers"
)
parser.add_argument("--resume_epoch", type=int, default=0, help="resume from epoch #")
parser.add_argument("--batch_size", type=int, default=24, help="batch size")
parser.add_argument("--min_views", type=int, default=1, help="minimum number of views")
parser.add_argument("--max_views", type=int, default=5, help="maximum number of views")
...
if not "train_set" in locals():
    ...
    print(
        "Reading the train image info took "
        + str(round(t2_ImageFolder - t1_ImageFolder, 2))
        + " seconds"
    )
else:
...
if not "val_set" in locals():
    ...
    t2_ImageFolder = time.time()
    print(
        "Reading the val image info took "
        + str(round(t2_ImageFolder - t1_ImageFolder, 2))
        + " seconds"
    )
else:
...
print(
    "total train models: {}; total train batches: {}".format(
...
print(
    "total val models: {}; total val batches: {}".format(len(val_set), len(val_loader))
)
...
encoder = network.Encoder().to(device)
convrnn = network.ConvRNN3d().to(device)
decoder = network.Decoder().to(device)
    ...
    encoder.load_state_dict(
        torch.load(
            os.path.join(
                saved_models_path,
                experiment_name,
                "encoder_{}_{:08d}.pth".format(s, epoch),
            )
        )
    )
    convrnn.load_state_dict(
        torch.load(
            os.path.join(
                saved_models_path,
                experiment_name,
                "convrnn_{}_{:08d}.pth".format(s, epoch),
            )
        )
    )
    decoder.load_state_dict(
        torch.load(
            os.path.join(
                saved_models_path,
                experiment_name,
                "decoder_{}_{:08d}.pth".format(s, epoch),
            )
        )
    )
...
def save(index, epoch=True):
    ...
    torch.save(
        encoder.state_dict(),
        os.path.join(
            saved_models_path, experiment_name, "encoder_{}_{:08d}.pth".format(s, index)
        ),
    )
    ...
    torch.save(
        convrnn.state_dict(),
        os.path.join(
            saved_models_path, experiment_name, "convrnn_{}_{:08d}.pth".format(s, index)
        ),
    )
    ...
    torch.save(
        decoder.state_dict(),
        os.path.join(
            saved_models_path, experiment_name, "decoder_{}_{:08d}.pth".format(s, index)
        ),
    )
...
hidden = (
    torch.zeros((batch_size, 128, 4, 4, 4)).to(device),
    torch.zeros((batch_size, 128, 4, 4, 4)).to(device),
)
...
for epoch in range(last_epoch + 1, num_epochs + 1):
    train_loader_pbar = tqdm(
        train_loader, desc=f"Epoch {epoch}/{num_epochs}", unit="batch"
    )
    for batch, data in enumerate(train_loader_pbar):
        it += 1
        hidden[0].detach_()
        hidden[1].detach_()
        train_loader_pbar.set_postfix({"batch": batch + 1})
        solver.zero_grad()
        ...
        solver.step()
    scheduler.step()
    mean_val_loss, mean_val_iou = validate(
        val_loader, encoder, convrnn, decoder, device
    )
