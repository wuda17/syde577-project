...
def loader_label(path):
    ...
    # Ensure voxel data is in dense numpy array form
    data = voxel.data

    # target dims expected by the network
    target = (32, 32, 32)
    ...
    if data.shape != target:
        ...
        # Nearest-neighbor downsampling: map target indices to source indices
        ix = np.minimum((np.arange(tx) * sx) // tx, sx - 1)
        iy = np.minimum((np.arange(ty) * sy) // ty, sy - 1)
        iz = np.minimum((np.arange(tz) * sz) // tz, sz - 1)

        # Use fancy indexing to sample
        resampled = src[np.ix_(ix, iy, iz)]

        # Create a new Voxels-like object to keep metadata (reuse original translate/scale/axis)
        voxel = type(voxel)(
            resampled, list(target), voxel.translate, voxel.scale, voxel.axis_order
        )
